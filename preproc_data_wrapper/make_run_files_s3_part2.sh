...
dataset=$1
in_bucket="s3://lifespan-seg-model-data/raw3"
out_bucket="s3://lifespan-seg-model-data/preprocessed3/T1-only"
data_dir="/tmp"
run_folder=`pwd`

preproc_folder="${run_folder}/run_files_${dataset}.preproc_data_part2"
preproc_template="template.preproc_data_part2"
...
done < <(s3cmd ls "${in_bucket}/images_${dataset}_renamed_relabeled/" | awk '{print $4}')
...
done < <(s3cmd ls "${in_bucket}/labels_${dataset}_renamed_relabeled/" | awk '{print $4}')
