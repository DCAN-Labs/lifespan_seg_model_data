...
def relabel_segmentation(input_file, output_file, create_lut=False):
    ...
    if create_lut:
        create_color_lut(valid_labels_with_rgb, "segmentation_lut.txt")
    ...
    if input_file.endswith('.nii.gz'):
        ...
        # Ensure the datatype is set correctly in the header
...
def main():
    ...
    parser.add_argument('--create-lut', action='store_true', help='Create a color lookup table file')
    ...
    # Run the relabeling
    num_relabeled = relabel_segmentation(args.input, output_file, args.create_lut)
