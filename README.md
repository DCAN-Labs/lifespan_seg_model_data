...
1. `cd` into the `preproc_data_wrapper` folder, edit the following variables in the `make_run_files_s3_part1.sh` file if needed: `in_bucket` and `out_bucket`. `in_bucket` likely wont need to be edited though, as the variable is typically just `"s3://lifespan-seg-model-data/raw"`. But if you are rerunning it is always best practices to save output onto a new location in the s3 for `out_bucket`. 
...
3. Check the `resources_preproc_data_part1.sh` to make sure parameters are properly specified. Parameters that may need editing in particular are `--mail-user`, `-p`, `-o`, `-e`, and `-A`.
4. Submit the jobs with the following command: `./submit_preproc_data_part1.sh 0-10 test`. The two additional arguments are an example array and example name for the dataset. The final outputs from our example will be here: `${out_bucket}/images_test_renamed_relabeled/` and `${out_bucket}/labels_test_renamed_relabeled/`
...
1. Within the `preproc_data_wrapper` folder, edit the following variables in the `make_run_files_s3_part2.sh` file if needed: `in_bucket` and `out_bucket`. The `in_bucket` should point to your recently renamed and relabeled data, and the `out_bucket` should be something like `"s3://lifespan-seg-model-data/preprocessed/T1-only"`.
2. Execute the `make_run_files_s3_part2.sh` script. This will require you to be in the `preproc_data_wrapper` directory and one argument identifying the dataset name. So based on our example above, the command will look something like `./make_run_files_s3_part2.sh test`. This will then make a unique run file based on the template `template.preproc_data_part2` for whatever file is available within `${in_bucket}/labels_${dataset}_renamed_relabeled/`, and the run files will then be stored in a folder called `./run_files_test.preproc_data_part2`.
3. Check the `resources_preproc_data_part2.sh` to make sure parameters are properly specified. Parameters that may need editing in particular are `--mail-user`, `-p`, `-o`, `-e`, and `-A`.  
4. Submit the jobs with the following command: `./submit_preproc_data_part2.sh 0-10 test`. The two additional arguments are an example array and example name for the dataset. The final outputs from our example will be here: `${out_bucket}/images/` and `${out_bucket}/labels/`
